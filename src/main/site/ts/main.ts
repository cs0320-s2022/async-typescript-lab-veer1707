...
const firstInput = document.querySelector('#sun') as HTMLInputElement
const secondInput = document.querySelector('#moon') as HTMLInputElement
const thirdInput = document.querySelector('#rising') as HTMLInputElement
const fourthInput = document.querySelector('#suggestions') as HTMLInputElement


// Here, when the value of sun is changed, we will call the method postAndUpdate.
// TODO: Do the same for moon and rising
firstInput.addEventListener("change", postAndUpdate);
secondInput.addEventListener("change", postAndUpdate);
thirdInput.addEventListener("change", postAndUpdate);
...
function postAndUpdate(): void {
  ...
  const postParameters = {
    ...
    //  HINT: use sun.value to get the value of the sun field, for example
      sun: firstInput.value,
      moon: secondInput.value,
      rising: thirdInput.value
  };
...
function updateSuggestions(matches: string[]): void {
  ...
  //  This makes each element selectable via screen reader.
    ...
    for (const match of matches) {
        fourthInput.innerHTML += '<li tabindex="0">' +
            match +
            '</li>';
    }
}
...
const values:MatchesRequestData = {
    sun: "Cancer",
    moon: "Libra",
    rising: "Aquarius"
}
document.addEventListener("keyup", event => {
    if (event.key === "Enter") {
        updateValues(values.sun, values.moon, values.rising).then(r => postAndUpdate());
    }
})
...
async function updateValues(sunval: string, moonval: string, risingval: string): Promise<void>{
  ...
  firstInput.value = sunval;
  secondInput.value = moonval;
  thirdInput.value = risingval;
}
